...
class MainWindow(QtWidgets.QMainWindow):
	...
	def open_systemDialog(self, position):
		...
		self.sysList_info[position] = [self.sysList[position].Systemnumber, self.sysList[position].PIUconf, self.sysList[position].Lposition, self.sysList[position].PUnumber,
							self.sysList[position].Monitormodel, self.sysList[position].Monitor2model, self.sysList[position].ECGnumber, self.sysList[position].Aquanumber, self.sysList[position].Aquamax]
	...
	def exportXML(self, fileLocation):
		try:
			...
			fieldofSys = ['SystemNumber', 'PIUconf', 'LocationpadNumber','PatchUnitNumber', 'Monitor1', 'Monitor2', 'ECGphantom', 'AquaNumber', 'AquaMaximu']
	...
	def format(self, type, position):
		if type == 'system':
			return 'System #%d: \nSystem Number:\t\t%s\nPIU Configuration:\t%s\nLocation Pad:\t\t%s\nPatch Unit:\t\t%s\nMonitor 1:\t\t%s\nMonitor 2:\t\t%s\nECG Phantom:\t\t%s\nAquarium Number:\t%s\nAquarium Maximo:\t%s\n-------------------\n' % (position + 1, self.sysList_info[position][0], self.sysList_info[position][1], self.sysList_info[position][2], self.sysList_info[position][3], self.sysList_info[position][4],self.sysList_info[position][5], self.sysList_info[position][6], self.sysList_info[position][7], self.sysList_info[position][8])
		if type == 'ultra':
	...
	def importBase(self, filelocation):
		...
		for child in importedroot:
			if (child.tag != 'Catheters' and child.tag != 'Extenders' and child.tag != 'Workstations' and child.tag != 'Header'):
